...
```{r}
# Use trans (transposed data) as data and meta for metadata
genScat <- function(gene, numeric, data, metadata) {
  ...
  #convert Multiple Columns to Numeric in R, updated with Chatgpt innput on way to achieve this that is not deprecated (without mutate_at())
  #access col names using strings with !!sym(Chatgpt)
  selm <- selm %>% mutate(across(c(!!sym(gene), !!sym(numeric)), as.numeric))
...
```{r}
genScat("ABCB10", "ferritin.ng.ml.", trans, meta)
```
